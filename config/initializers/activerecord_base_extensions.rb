...
class ActiveRecord::Base
  ...
  def init_token
    self.token = self.class.friendly_token if self.has_attribute?(:token) && self.token.nil?
  end
  ...
  def owned?
    User.current && (User.current.id == self.user_id || User.current.id == supervisor_id)
  end
 ...
 def self.debug(*args)
  ...
  def debug(*args)
    return unless Rails.env = "development"
    title ||= ""
    logger.ap '-' * 20 + calling_method + '- in model' * 20
    logger.ap args
    logger.ap '-' * 100
  end
